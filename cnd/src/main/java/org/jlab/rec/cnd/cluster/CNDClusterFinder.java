...
public class CNDClusterFinder {
	...
	public ArrayList<CNDCluster> findClusters(ArrayList<CndHit> hits) {
		...
		for (int i = 0; i < size; i++) {
			...
			if(hits.get(i).get_tX()!=0 && hits.get(i).get_tY()!=0 && hits.get(i).get_tZ()!=0
			   && (hits.get(i).Z() - hits.get(i).get_tZ())>9
			   && hits.get(i).Z()>(CalibrationConstantsLoader.LENGTH[hits.get(i).Layer()-1]-CalibrationConstantsLoader.ZOFFSET[hits.get(i).Layer()-1]) ) continue;
